...
export const PracticeLayout: React.FC<PracticeLayoutProps> = ({ children }) => {
    ...
    useEffect(() => {
        const fetchProblems = async () => {
            setLoading(true);
            try {
                const data = await loadPracticeProblems(normalizedTopic);
                setProblems(data);
            } catch (error) {
            ...
            <AnimatePresence mode='wait'>
                {(isSidebarOpen || window.innerWidth >= 1024) && (
                    <motion.aside
                        ...
                        className="fixed inset-y-0 left-0 z-40 w-80 bg-white/80 backdrop-blur-xl border-r border-[#e5e5e5] flex flex-col lg:relative lg:inset-auto h-full"
                    >
                        {/* Header */}
