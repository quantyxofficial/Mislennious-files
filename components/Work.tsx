...
export const Work: React.FC = () => {
  ...
  // Precise mapping: When we scroll down 100% of the section height, move left by 65%
    ...
    <div id="work" className="relative bg-transparent">
      ...
      <section ref={targetRef} className="hidden lg:block relative h-[300vh] bg-transparent">
        <div className="sticky top-0 flex h-screen items-center overflow-hidden z-10">
            ...
            {/* Section Header - Increased Visibility */}
            <div className="absolute top-8 left-6 lg:left-32 z-20 pointer-events-none">
                <h2 className="font-serif text-6xl md:text-9xl text-lux-text/15 uppercase tracking-tighter">Selected Works</h2>
            </div>

            <motion.div style={{ x }} className="flex gap-12 px-6 lg:px-32 items-center will-change-transform">
            {/* Intro Card */}
            ...
            {CASE_STUDIES.map((project) => (
                <div 
                key={project.id} 
                className="group relative h-[60vh] md:h-[70vh] w-[85vw] md:w-[60vw] lg:w-[50vw] flex-shrink-0 bg-white rounded-[3rem] overflow-hidden shadow-2xl shadow-stone-200/50 border border-white/60"
                data-hover
                >
