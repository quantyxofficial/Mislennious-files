...
export const PracticeLayout: React.FC<PracticeLayoutProps> = ({ children }) => {
    ...
    return (
        <div className="flex h-screen bg-black overflow-hidden pt-4">
            {/* Mobile Toggle */}
            <button
            ...
            {(isSidebarOpen || window.innerWidth >= 1024) && (
                <aside className="fixed inset-y-0 left-0 z-40 w-80 bg-black/80 backdrop-blur-xl border-r border-white/5 flex flex-col lg:relative lg:inset-auto h-full">
                    {/* Header */}
                    <div className="p-8 border-b border-white/5">
                        <Link to={`/practice/${topic}`} className="text-[10px] font-bold uppercase tracking-[0.2em] text-lux-muted hover:text-white transition-colors mb-3 block">
                            &larr; Return to Overview
                        </Link>
                        <h2 className="text-3xl font-serif text-lux-text tracking-tight">{normalizedTopic}</h2>
                        <p className="text-xs text-lux-muted mt-2 font-medium tracking-wide font-sans">{problems.length} Challenges Available</p>
                    </div>
                                                ...
                                                <Link
                                                    ...
                                                    className={`group flex items-center justify-between px-4 py-3 rounded-lg text-sm transition-all duration-300 border
                                                        ${isActive
                                                            ...
                                                            : 'bg-transparent text-lux-muted border-transparent hover:bg-white/5 hover:text-white'
                                                        }`}
                                                >
